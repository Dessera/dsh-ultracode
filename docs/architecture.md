...
- While the level is not `off`, it raises the reasoning effort of that session's requests to the
  strongest value the route reported when the pin was computed.

| Level   | Instruction injected after the last human message of a substantive turn                                                                   | Reasoning effort                                                             |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `off`   | Nothing is injected.                                                                                                                      | No pin, and returning to `off` releases the effort the plugin raised.        |
| `high`  | The `high` block: a few parallel perspectives, then one adversarial refutation pass.                                                      | Pinned to the strongest effort the route reported when the pin was computed. |
| `ultra` | The `ultra` block: a wide fan-out, rounds that stop only after two consecutive rounds find nothing new, and a closing completeness check. | Pinned to the strongest effort the route reported when the pin was computed. |

Two consequences of that table are worth stating plainly, because they are easy to misread.

First, the injected text is the only difference between `high` and `ultra`. Both pin the same
...
| Half   | Source          | Artifact        | Runtime                                                                                                        |
| ------ | --------------- | --------------- | -------------------------------------------------------------------------------------------------------------- |
| Host   | `src/host/**`   | `lib/index.js`  | Node ESM, loaded by the plugin's loader row in the host process.                                               |
...
The host half and the client half never call each other. They agree on a vocabulary and a wire
shape, and the harness carries values between them.
...
| `src/host/protocol.ts`            | The vocabulary both halves share: plugin id, command name, projection key, level vocabulary and aliases, rotation rule, wire shape, and the hand-written parsers.                                  |
...
| `src/host/effort.ts`              | `EffortResolver`, which decides which effort to pin, and `withEffortPlan`, which applies one decision to a call configuration.                                                                     |
...
| `src/client/index.ts`             | Client entry point. Registers the dictionaries and the composer seat.                                                                                                                              |
...
| `src/client/contract.ts`          | The seat props type and the locale-namespace merge.                                                                                                                                                |
...
per-session record seeded once from the fold. It exists so that both synchronous decisions stay
...
registry still gets the command channel, the banner injection and the effort pin; it keeps the
...
the reasoning-effort field. The value it writes was computed once per arming, so a model switch made
...
guarantees that order only as the adapter's preferred display order: the pin assumes the adapter lists
its efforts weakest first, and a route that lists them strongest first makes the pin select the weaker
entry. Lookups are cached per provider and model, including failures, and a route that reports no
usable effort produces no pin at all.
...
- if the plugin captured a baseline before its first pin, that value is written back;
- if there is no baseline — the state of a session whose level came back from the log after a host
  ...
  so the request falls back to the model's own default rather than keeping the pinned effort.
...
| `off`, `none`, `close`, `关闭` | Switch the level off and release the effort pin.                         | The `off` notice.                                                                                                                                                                           |
...
notifies its change feed when that view changes; the session-control carrier turns that notification
...
The client half registers one entry in the composer's right-hand tool row. The entry joins the list
...
- **Read.** The seat receives DSH's projection hook as a prop. The control calls it with the key from
  `protocol.ts` and renders the value it gets back, after checking that the reply is an object: a
  reply that is not an object, or a hook that throws, is treated as nothing published yet. It keeps
  no cache, fetches nothing, and holds no level of its own.
...
already in flight, when the session cannot carry the command, or when the seat carries no session id
to address the command to; the control renders nothing at all for a session that was removed or for a
...
table for the case where the seat is rendered without this plugin's locale namespace registered.
...
```mermaid
flowchart TB
    subgraph browser[Browser half]
        seat["Seat prop: the projection hook"]
        chip[Composer chip]
    ...
    request -- "pin or release the effort" --> call
```
...
- `pnpm check` runs lint, typecheck, build and tests in that order. The bundle tests assert against
  `lib/client.js`, so the client half must be built before those tests run.
...
| `test/config-effort.test.mjs`    | Configuration resolution and the effort planner, including the pin, the release and the field-clearing plan.                                                                             |
...
| `test/host-integration.test.mjs` | The built host bundle driven through a stubbed context: both waterfalls, the command, the projection, and the effort pin and release.                                                    |
...
| `test/contract-probe.test.mjs`   | The names the plugin resolves out of the installed harness, which the compiler cannot check, and the rule that the harness type packages are pinned to exact versions in `package.json`. |
...
- **The plugin pins effort on top-level sessions only.** Its `agent/request` listener skips every
  ...
  is armed that recorded effort is the pinned one. The inherited effort is dropped only when the
...
- **A model switch does not re-resolve the pin.** The plan is computed once per arming, so a session
  that switches models while its level stays armed keeps asking for the effort id of the route it was
  armed on. When the new model does not declare that effort, the harness refuses the request before
  any provider I/O; returning to `off` and arming again re-resolves it.
- **A raise that is neither recorded nor remembered is not possible.** The result of the
  ...
  The only unrecorded path is a value the adapter materializes itself, and that path cannot be forced
...
- **`high` and `ultra` share one effort pin.** Their only difference is the injected instruction
  block.
- **The level is the only arming path.** There is no separate trigger, and the command surface is
