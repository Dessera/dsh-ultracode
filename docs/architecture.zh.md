...
插件让每个会话拥有一组三个可选的档位。档位是插件唯一的产品状态，它只有一个效果：决定插件注入到一次实质性用户回合里的指令块是哪一块，注入位置紧接在人写下的最后一条消息之后。

| 档位    | 在实质性回合的最后一条人类消息之后注入的指令                                             |
| ------- | ---------------------------------------------------------------------------------------- |
| `off`   | 不注入任何内容。                                                                         |
...
第一，注入文本是 `high` 与 `ultra` 之间唯一的差别。插件对两档的门控条件完全相同，两者都让模型可以自由地直接作答，而不是必须调用 workflow 工具。
