...
| 档位    | 在回合开局消息之后注入的指令                                                                   |
| ------- | ---------------------------------------------------------------------------------------------- |
| `off`   | 不注入任何内容，除非这个回合紧跟在档位被关闭之后，那种情况下它会拿到一条「模式已结束」的提示。 |
...
关闭档位是唯一一种开启提示报告不了的变化，因为开启提示只为开启档位的回合注入。紧跟这次变化的那个回合因此会拿到由 `buildDisarmNotice` 拼装的提示，它说明模式已经结束，并说明这个回合不授权任何 workflow 运行。这条提示只付一次：一个开启过的档位结束后，该会话就欠着它；真正把它注入出去的那个回合把这笔欠账结清；重新开启档位则把它作废。

这张表带来三个结论，值得直说，因为它们容易被读错。

第一，档位是唯一的门控条件。插件不看消息的长短，也不判断它是不是一个任务：一个提问、一句「继续」、一次自动续跑，都和其他回合一样拿到注入文本。回合里没有开局消息时不注入，因为那一步只是本回合自己的延续。关闭提示受同样的门控：没有开局消息的那一步不会把它花掉，而是让它继续欠着。
...
| `src/host/state.ts`               | 按会话划分的内存镜像（`UltracodeStateStore`），其中也包括关闭提示要偿还的那笔一次性欠账。              |
| `src/host/prompt.ts`              | 开启提示的文本与各档位指令块的拼装、重复回合用的那一行简短提醒，以及关闭档位后的回合拿到的提示。       |
...
- 镜像报告的档位不是 `off`，或者该会话还欠着一条关闭提示，
- 对于开启档位的回合，配置指定的 workflow 工具在该会话的工具作用域里能解析到，
...
档位被关闭之后的那个回合拿到的是由 `buildDisarmNotice` 拼装的提示，它是普通文本而不是被方括号包起来的开启块：此刻什么都没被开启，而这条提示自己第一句就是这么说的。它不携带任何档位指令，所以读它的模型不会在里面看到 `Effort: HIGH` 或 `Effort: ULTRA`；它的来源摘要由 `disarmSummary` 写成 `ultracode mode ended before this turn`。这条摘要刻意一个档位词都不含，连 `off` 都不含：折叠是靠寻找档位词从摘要里读档位的，因此一条写着 `high` 或 `ultra` 的提示会被读成一次开启，而一条写着 `off` 的提示会让一份本来不带档位命令的日志回退成 `off`，而不是让先前开启提示确立的档位保持原样。

这条提示由那次档位变化欠下、并且只还一次。一个非 `off` 的档位变成 `off` 时，`UltracodeStateStore.select` 记下这笔欠账；真正注入出去的那个回合把它结清；任何在注入之前就把跳过理由解决掉的步骤——被拒绝的一步、没有开局消息的一批、这一批里找不到锚点——都让它继续欠着。重新开启档位会把它作废，因为一个重新开启的会话由新档位的开启提示服务，在旁边再注入一段关于已结束档位的陈旧说明会与那条提示自相矛盾。被重新放回收件箱的开局消息只会收到一次提示，这份幂等由认领机制保住，它给关闭提示用的是和开启提示同样的锚定方式。与「已宣告过的档位」一样，这笔欠账记在宿主内存里而不入库：重启后的宿主从折叠里恢复档位，而在重启之前发生的那次变化不会被任何东西报告出来。

**每回合只注入一次**由锚点消息的标识决定，而不是由回合号决定。这是有意的：人在智能体干活时补一句话（`steer`）会并入正在跑的那个回合，回合号不变，但消息是新的，所以它照样拿到自己的那条开启提示。关闭提示同理，它用的是自己独立的锚点字段，因此一次关闭提示不会让后来的开启提示看起来已经注入过。
